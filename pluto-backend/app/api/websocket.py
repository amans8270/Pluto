...
async def _authenticate_ws(token: str, expected_user_id: str) -> str:
    """
    Verify Supabase token AND validate it matches the user_id in the URL.
    Returns supabase_uid on success.
    """
    try:
...
async def websocket_endpoint(
    websocket: WebSocket,
    user_id: str,
    token: str = Query(..., description="Supabase ID token"),
):
    """
    Main WebSocket endpoint.
    Client connects to: wss://api.pluto.app/ws/{user_id}?token=<supabase_id_token>
    """
    # Authenticate AND verify user_id matches token
    try:
        supabase_uid = await _authenticate_ws(token, user_id)
